...
All requests require HTTP Basic Auth. Pass your API key and secret as Base64-encoded credentials in the `Authorization` header.
...
<ParamField query="include_content" type="boolean" default="false">
  When `true`, each post in the response includes `content` (raw JSON string), `content_json` (parsed content blocks), and `content_html` (rendered HTML). Omit for list views where full body content is not needed.
</ParamField>
...
```bash
curl -X GET \
  'https://cds.thepublive.com/publisher/<PUBLISHER_ID>/posts/trending/?duration=24h&limit=10' \
