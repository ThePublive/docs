...
<Update label="July 28, 2026" tags={["New releases","Security","Improvements","Bug fixes"]} rss={{ title:"Safer Facebook login, split API keys, a Google Discover Headline pilot, and dashboard fixes" }}>
  ### Google Discover Headline

  The OG Title field has moved out of the SEO Properties panel and into a dedicated **Google Discover Headline** field, shown directly under Title.

  - Same underlying value as the old OG Title field — existing headlines carry over automatically, nothing is lost.
  - Controls only the headline shown when your content appears in Google Discover, not your page's Title or Meta Title.
  - Live behind a feature flag for a small group of pilot publishers — contact your account team to join the pilot.
  ...
  - **Form acknowledgement emails** — emails now show clear, readable field labels for a better reader experience.
  - **Analytics** — the dashboard **Insights** link now takes you straight to the new analytics dashboard.
  - **Author profile mail icon** — readers can now reach any author directly, with a mail icon live on every author profile.
  - **Form permissions** — now apply consistently everywhere in the dashboard, exactly as configured.

  ### Bug Fixes

  - **Heading hierarchy** — semantic heading structure (H1, H2, H3) across homepage, section, tag, and author pages is now consistent, improving SEO and accessibility.
</Update>
