...
Sets a new password for a reader using the one-time token sent by [Forgot password](/dxp/api-reference/content-management/reader/forgot-password). `new_password` and `confirm_password` are validated server-side — the request returns `400` if they do not match.
