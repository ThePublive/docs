...
<Note>
  After a successful registration, guide the reader to check their email. Pass the `email` and `token` from the verification link to [Verify email](/dxp/api-reference/content-management/reader/verify-email) to activate the account.
</Note>
