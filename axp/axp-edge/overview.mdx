...
```mermaid
flowchart LR
    A[Request arrives at your CDN] --> B{AXP Edge identifies the requester}
    B -->|Human visitor| C[Your site, untouched]
    B -->|Search crawler e.g. Googlebot, Bingbot| C
    B -->|AI agent e.g. GPTBot, PerplexityBot| D[Agent-first HTML]
    D --> E[JS rendered, token-optimized, clean structure]
```
