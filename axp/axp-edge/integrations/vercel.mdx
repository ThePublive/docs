...
This guide adds a Next.js Middleware that routes AI agent traffic (GPTBot, ClaudeBot, Google-Extended, and similar) to the Publive CDS backend, while human visitors and SEO bots continue to hit your app exactly as before. It ships as part of your normal Next.js build — no separate infrastructure to stand up.
...
- Your Publive Edge CDS API key and backend URL (issued per client — contact [Publive](https://axp.thepublive.com/contact) if you don't have these).
- The [Vercel CLI](https://vercel.com/docs/cli), if you plan to test Middleware locally (see [Local development](#local-development)).

## How routing works

Middleware runs on every request inside your Next.js app, ahead of routing. It passes `_next/` paths through untouched, returns a 404 directly for any `.well-known/` path, and delegates every other request's eligibility decision to a shared `routeToPublive()` module. A request only qualifies for AXP Edge when **all** of the following are true:
...
<Warning>
  The `.well-known/` handling in the code below returns a 404 for **every** path under `.well-known/`, unconditionally. If your app serves anything from that path — ACME HTTP-01 challenge files, `apple-app-site-association`, `security.txt`, third-party domain verification files — this Middleware will block it before your app ever sees the request. Remove or adjust that block if you rely on any `.well-known/` route.
</Warning>
...
<Steps>
  ...
  <Step title="Add the routing module">
    ...
    async function routeToPublive(request, env) {
      ...
      if (!env.NEXT_PL_EDGE_API_KEY || !env.NEXT_PL_AXP_EDGE_BACKEND_URL) {
        console.error("[error] NEXT_PL_EDGE_API_KEY or NEXT_PL_AXP_EDGE_BACKEND_URL not configured for this worker");
        debugLog(env, "skip:missing-config", {
          hasApiKey: !!env.NEXT_PL_EDGE_API_KEY,
          hasBackendUrl: !!env.NEXT_PL_AXP_EDGE_BACKEND_URL,
        });
        return null;
      }

      const headers = buildCDSHeaders(request, url, env, preview);
      debugLog(env, "fetching-cds", { backendUrl: env.NEXT_PL_AXP_EDGE_BACKEND_URL });

      try {
        const response = await fetch(
          new Request(env.NEXT_PL_AXP_EDGE_BACKEND_URL, {
  ...
  <Step title="Configure environment variables">
    ...
    | `NEXT_PL_AXP_EDGE_BACKEND_URL` | Publive CDS backend URL. | Yes | `https://cds.thepublive.com/axp/view/` |
  ...
  <Step title="Deploy">
    Ship the Middleware as part of your normal app build/deploy pipeline — there's no separate infrastructure to stand up.
  </Step>
</Steps>
...
- **Bot traffic** — a request with an AI agent user-agent returns a response with the CDS routing confirmation header present.
...
| `skip:missing-config` | `NEXT_PL_EDGE_API_KEY` or `NEXT_PL_AXP_EDGE_BACKEND_URL` not set | Add the missing variable and redeploy/restart |
...
| `skip:fetch-threw` | Network error reaching CDS | Check `NEXT_PL_AXP_EDGE_BACKEND_URL` is reachable |
